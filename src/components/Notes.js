...
function Notes(props){
    ...
    return(
        <div className="notes" style={{width: props.width+"px"}}>
            <div className="notes__head">
                <h2 className="notes__title">Заметки</h2>
                <button className="notes__add-button" onClick={props.onClick}></button>
            </div>
            <input className="notes__input" placeholder="Найти заметку по имени" onChange={(e)=>props.handleSearchNote(e.target.value)}/>
